...
def __process_spanish_sentences(
    ...
    for example: "1276\tspa\tLet's try something.\n"
    ...
    for line in in_file:
        line = line.split('\t')
        # use only Spanish sentences
        if len(line) > 1 and line[1] == 'spa':
            line = line[2].strip()
            if re.match("^[A-Z][A-Za-z.,'?¿¡ÁÉÍÓÚÜÑáéíóúüñ\s]+$", line):  # nopep8
...
if __name__ == "__main__":
    ...
    logging.info(f'Processing Spanish sentences...')
    clean_sentences = os.path.join(args.data_dir, 'clean_sentences.txt')
    __process_spanish_sentences(
        tatoeba_dataset, clean_sentences, args.percent_to_cut, args.num_lines_to_combine, args.num_samples
    )
    ...
    __split_into_train_dev(clean_sentences, train_file, dev_file, args.percent_dev)
